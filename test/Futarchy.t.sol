...
abstract contract FutarchyMarketTestBase is Test, Events, Types {
...
contract CreateMarketTest is FutarchyMarketTestBase {
    ...
    function test_WhenCallerIsDAOAndMarketDoesntExist() external {
        ...
        // no idea why removing custom errors from openzeppelin breaks this
        // emit MarketCreated(proposalId, tradingPeriod);
        emit MarketCreated(proposalId, tradingPeriod);

        market.createMarket(proposalId, questionHash, tradingPeriod);
        MarketInfo memory marketInfo = market.getMarketInfo(proposalId);
        ...
        assertEq(uint256(marketInfo.outcome), uint256(Outcome.Unresolved));
...
contract BuySharesTest is FutarchyMarketTestBase {
    ...
    function test_RevertWhen_MarketIsAlreadyResolved() external {
        vm.warp(block.timestamp + tradingPeriod + 1);
        oracle.resolveMarket(address(market), proposalId, Outcome.Yes);
    ...
    function test_WhenAllConditionsAreMet() external {
        ...
        MarketInfo memory marketInfoBefore = market.getMarketInfo(proposalId);
        ...
        //     buyAmount
        // );
        emit SharesBought(proposalId, alice, true, sharesToMint, buyAmount);
        ...
        Position memory alicePosition = market.getPosition(proposalId, alice);
...
contract SellSharesTest is FutarchyMarketTestBase {
    ...
    function test_WhenAllConditionsAreMet() external {
        Position memory alicePositionBefore = market.getPosition(proposalId, alice);
        ...
        emit SharesSold(proposalId, alice, true, sellAmount, expectedTokensToReceive);
        ...
        Position memory alicePositionAfter = market.getPosition(proposalId, alice);
        assertEq(
            alicePositionAfter.yesShares, alicePositionBefore.yesShares - sellAmount, "Shares not burned from user"
        );

        MarketInfo memory marketInfoAfter = market.getMarketInfo(proposalId);
        assertEq(
...
contract ResolveMarketTest is FutarchyMarketTestBase {
    ...
    function test_RevertWhen_CallerIsNotOracle() external {
        ...
        market.resolveMarket(proposalId, Outcome.Yes);
    }
    ...
    function test_RevertWhen_MarketDoesntExist() external {
        ...
        vm.expectRevert("Market does not exist");
        market.resolveMarket(nonExistentProposalId, Outcome.Yes);
    }
    ...
    function test_RevertWhen_MarketIsAlreadyResolved() external {
        ...
        vm.prank(address(oracle));
        vm.expectRevert("Market is already resolved");
        market.resolveMarket(proposalId, Outcome.No);
    }
    ...
    function test_WhenAllConditionsAreMet() external {
        ...
        vm.expectEmit(true, true, true, true);
        // emit MarketResolved(proposalId, Outcome.Yes);
        emit MarketResolved(proposalId, Outcome.Yes);

        market.resolveMarket(proposalId, Outcome.Yes);

        MarketInfo memory resolvedMarket = market.getMarketInfo(proposalId);

        assertEq(uint256(resolvedMarket.outcome), uint256(Outcome.Yes), "Incorrect outcome");
    ...
    function test_ResolvingWithDifferentOutcomes() external {
        ...
        MarketInfo memory resolvedMarket = market.getMarketInfo(proposalId);
        assertEq(uint256(resolvedMarket.outcome), uint256(Outcome.No), "Incorrect outcome");
        ...
        vm.warp(block.timestamp + tradingPeriod + 1);
        vm.prank(address(oracle));
        market.resolveMarket(proposalId2, Outcome.Unresolved);

        MarketInfo memory resolvedMarket2 = market.getMarketInfo(proposalId2);
        assertEq(uint256(resolvedMarket2.outcome), uint256(Outcome.Unresolved), "Incorrect outcome");
    }
...
contract ClaimWinningsTest is FutarchyMarketTestBase {
    ...
    function test_WhenAllConditionsAreMet() external {
        ...
        emit WinningsClaimed(proposalId, alice, expectedWinnings);
        market.claimWinnings(proposalId);
    ...
    function test_ClaimWinningsWithDifferentOutcomes() external {
        ...
        vm.warp(block.timestamp + tradingPeriod + 1);
        oracle.resolveMarket(address(market), proposalId, Outcome.No);
...
contract GetPositionTest is FutarchyMarketTestBase {
    ...
    function test_WhenMarketExists() external {
        ...
        Position memory bobPosition = market.getPosition(proposalId, bob);
        ...
        Position memory charliePosition = market.getPosition(proposalId, charlie);
    ...
    function test_PositionAfterSellingShares() external {
        ...
        Position memory positionBeforeSelling = market.getPosition(proposalId, alice);
        ...
        Position memory positionAfterSelling = market.getPosition(proposalId, alice);
        assertEq(
    ...
    function test_PositionInMultipleMarkets() external {
        ...
        Position memory alicePosition1 = market.getPosition(proposalId, alice);
        Position memory alicePosition2 = market.getPosition(proposalId2, alice);
